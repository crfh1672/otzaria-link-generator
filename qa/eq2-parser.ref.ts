...
import { expandAbbreviationsInText, DEFAULT_ABBREVIATIONS, NORMALIZED_ABBREVIATIONS_MAP } from './eq2-abbrev.ref';
import { getWordSimilarity, getNikudFingerprint, levenshteinDistance } from './baseline/fuzzyUtils.original';
...
function reNormalizedWords(entry: LineCacheEntry | undefined, norm: string): string[] {
  ...
  if (words === undefined) {
    ...
    words = reNorm.split(/\s+/).filter(Boolean);
  }
  return words;
}
...
export function runLinkingParser(
  ...
  const searchPrimaryWithFirstAnchor = (
    ...
    const expandedSegWords = (
      ...
      void slot; void lineIdx; void segWords; void segExpansionCache;
      const expanded = config.useAbbreviationExpansion !== false
        ? expandAbbreviationsInText(seg, contextNorm, abbrDict, config.gsReplacements)
        : seg;
      return normalizeText(expanded).split(/\s+/).filter(Boolean);
    };
  ...
  const searchLineInDoc = (
    ...
    const bestOfCombos = (
      ...
      const combos = [
        calcContiguousScore(source, target),
        calcContiguousScore(expSource, expTarget),
        calcContiguousScore(source, expTarget),
        calcContiguousScore(expSource, target),
      ];
      return combos.reduce((best, c) => c.score > best.score ? c : best);
    };
    ...
    for (const range of searchRanges) {
      ...
      for (let lNum = range.s; lNum <= range.e; lNum++) {
        ...
        const expDocWords = reNormalizedWords(cachedLine, expDocLineNorm);
